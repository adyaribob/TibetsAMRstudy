...
dat.fig2.lefse.arg.mge.vf.barchart <- d
write.csv()
fig.barchart.gene.lefse <- ggplot(d[d$lefse != "Nogroup", ], 
...
#### neutral model #####
a <- rbind(dat.neutral.water.gene.arg[,colnames(dat.neutral.water.gene.vf)], 
           dat.neutral.sediment.gene.arg[,colnames(dat.neutral.water.gene.vf)],
           dat.neutral.water.gene.mge[,colnames(dat.neutral.water.gene.vf)], 
           dat.neutral.sediment.gene.mge[,colnames(dat.neutral.water.gene.vf)], 
           dat.neutral.water.gene.vf[,colnames(dat.neutral.water.gene.vf)], 
           dat.neutral.sediment.gene.vf[,colnames(dat.neutral.water.gene.vf)])
a$domain <- factor(a$domain, levels = c("ARGs","MGEs","VFGs"))
a$media <- factor(a$media, levels = c("Water","Sediment"))
a$lefse %>% unique
a$lefse[is.na(a$lefse)] <- "None"
dat.fig2.neutral.model.gene <- a
write.csv(dat.fig2.neutral.model.gene, "dat.fig2.neutral.model.gene.csv", col.names = FALSE)

fig.neutral.genes <- ggplot(dat.fig2.neutral.model.gene, aes(x=log10(p), y=freq ))+
...
### barchart ####
b <- filter(dat.fig2.neutral.model.gene, domain %in% c("ARGs", "MGEs", "VFGs")) %>% 
...
b$domain
b$n <- as.numeric(b$n)
fig.neutral.genes.barchart <- ggplot(b, aes(x = lefse, y = n, fill = upper_neutral_under)) +
...
##### boxplot of abudnance of genes based on lefse in water and sediment #######
dat.fig2.boxplot.abundance 
dat.fig2.boxplot.abundance.sig.test 

ggplot(dat.fig2.boxplot.abundance, 
  ...
  xlab("")+
  geom_text(data = dat.fig2.boxplot.abundance.sig.test, 
